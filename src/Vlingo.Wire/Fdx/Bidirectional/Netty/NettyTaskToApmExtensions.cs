    ...
    public static class NettyTaskToApmExtensions
    {
        public static IAsyncResult BeginConnect(this Bootstrap bootstrap, string hostName, int port, AsyncCallback callback, object state)
        {
            if (hostName == "localhost")
            {
                ...
                return TaskToApm.Begin(bootstrap.ConnectAsync(new IPEndPoint(IPAddress.Loopback, port)), callback, state);
            }
            
            return TaskToApm.Begin(bootstrap.ConnectAsync(hostName, port), callback, state);
        ...
        public static IChannel EndConnect(this Bootstrap _, IAsyncResult asyncResult)
        {
            try
            {
                ...
                return TaskToApm.End<IChannel>(asyncResult);
            }
