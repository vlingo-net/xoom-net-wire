...
namespace Vlingo.Wire.Message
{
    public interface IConsumerByteBuffer
    {
        ...
        int Capacity { get; }
        int Position();
        IConsumerByteBuffer Position(int newPosition);
        int Limit();
        IConsumerByteBuffer Limit(int newLimit);
        ...
        Stream Get(byte[] destination, int offset, long length);
        char GetChar();
        ...
        IConsumerByteBuffer Put(byte[] src, int offset, long length);
